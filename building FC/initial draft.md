...
>`date: 11-03-2026 2250hrs`

## Connecting and Controlling Pixhawk via ST-Link

Since you're diving into the "bare-metal" side of your aerospace project, using an ST-Link is a professional move. It moves you away from being a "user" of firmware to being the **developer** of it.

### 1. Connecting via 4 Pins

Yes, you can absolutely connect using 4 pins. This is the standard **SWD (Serial Wire Debug)** configuration. You do not need a full 10-pin or 20-pin ribbon cable.

The 4 pins you need are:
...
4. **VCC/3.3V** (Target Voltage Reference - _Note: This tells the ST-Link what voltage the Pixhawk is running at._)
    

### 2. Identifying the IO and CLK Ports

On a Pixhawk (like the 2.4.8 or FMUv5), these aren't usually labeled "IO" and "CLK" on the plastic case. You have to look for the **6-pin or 10-pin "Debug" or "FMU Debug" port**.

If you are looking at a standard 6-pin SH Debug port, the pinout is typically:

- **Pin 1:** VCC (3.3V)
    
- **Pin 2:** SWDIO (This is your "IO")
    
- **Pin 3:** SWCLK (This is your "CLK")
    
- **Pin 4:** SWO (Optional - Trace output)
    
- **Pin 5:** GND
    
- **Pin 6:** nRST (Reset - useful if your code crashes the CPU)
...
Once you hit "Connect" in **STM32CubeProgrammer** on your Windows 11 machine, you have "God Mode" over the hardware. Here is what you can do:

#### A. Total Firmware Wipe (The "Clean Slate")

Unlike a USB update which keeps the bootloader, ST-Link allows you to perform a **Full Chip Erase**. This deletes every bit of PX4 or ArduPilot, leaving the STM32 chip completely blank and ready for your custom C code.

#### B. Step-by-Step Debugging (The Most Powerful Tool)

This is why engineers use ST-Link. In your IDE (like VS Code or STM32CubeIDE), you can:

- **Set Breakpoints:** Pause the code at a specific line (e.g., right after your Pitch/Roll calculation).
    
- **Inspect Variables:** Look at the exact value of your `float pitch_error` in real-time while the code is "frozen."
    
- **Step Into:** Execute your code one single line at a time to see exactly where a calculation might be going wrong.
    

#### C. Register Access

You can see the status of the **Registers**. For an aerospace student, this is vital for checking if your **PWM** signals for the motors are actually being generated by the timers, or if the **I2C/SPI** bus to your IMU is hanging.

#### D. Option Byte Modification

You can change the hardware-level configurations, such as "Read Out Protection" (to lock your code so others can't copy it) or watchdog settings.

#### E. Flashing without a Bootloader

You can upload your `.bin` file instantly. There is no waiting for a "bootloader timeout" like you have with USB. It flashes the memory directly at high speed.

---

### A Small Warning for your Hardware

Since you are using Windows 11, if the ST-Link isn't recognized, you may need to update the **ST-Link Firmware** itself. You can do this within the STM32CubeProgrammer tool by clicking the **"ST-LINK Upgrade"** button.
